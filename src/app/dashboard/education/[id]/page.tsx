...
export default function EditEducationPage() {
  ...
  const { getEducationById, updateEducation, loading } = useEducations();
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const education = await getEducationById(params.id as string);
        if (education) {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      await updateEducation(params.id as string, values);
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto py-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-2xl font-bold tracking-tight">Modifier la formation</h2>
      </div>
